...
def sample_tenants(app):
    with app.app_context():
        # Create test tenants
        zen_garden = Tenant(id=str(uuid4()), name="Zen Garden Yoga", subdomain="zengarden")
        consulting = Tenant(id=str(uuid4()), name="Bob's Consulting", subdomain="bobconsulting")
        db.session.add_all([zen_garden, consulting])
        db.session.commit()

        yield {"zen_garden": zen_garden, "consulting": consulting}
...
def test_user_tenant_role_creation(app, sample_tenants, sample_roles):
    ...
    with app.app_context():
        # Create test user
        user = User(
            id=str(uuid4()), email="maya@zengardenyoga.com", first_name="Maya", last_name="Chen"
        )
        db.session.add(user)
        db.session.commit()
        ...
        user.add_tenant_role(
            sample_tenants["zen_garden"], sample_roles["zen_owner"], is_primary=True
        )

        # Add Maya as client of consulting
        user.add_tenant_role(sample_tenants["consulting"], sample_roles["consulting_client"])
...
def test_primary_tenant_switching(app, sample_tenants, sample_roles):
    ...
    with app.app_context():
        ...
        user = User(id=str(uuid4()), email="test@example.com", first_name="Test", last_name="User")
        db.session.add(user)
        db.session.commit()

        # Add roles
        user.add_tenant_role(
            sample_tenants["zen_garden"], sample_roles["zen_staff"], is_primary=True
        )
        user.add_tenant_role(sample_tenants["consulting"], sample_roles["consulting_client"])
        ...
        zen_role = user.tenant_roles.filter_by(tenant_id=sample_tenants["zen_garden"].id).first()
...
def test_tenant_role_validation(app, sample_tenants, sample_roles):
    ...
    with app.app_context():
        ...
        with pytest.raises(ValueError):
            user.add_tenant_role(sample_tenants["zen_garden"], sample_roles["zen_staff"])
