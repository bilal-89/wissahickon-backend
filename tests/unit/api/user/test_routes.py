...
class TestUserRoutes:
    ...
    def ensure_user_tenant_role(self, user, tenant, permissions=None):
        ...
        user_role = db.session.execute(
            db.select(UserTenantRole).filter_by(user_id=user.id, tenant_id=tenant.id)
        ).scalar_one_or_none()
    ...
    def test_create_user(self, client, test_user_with_role, test_tenant):
        ...
        role = db.session.execute(db.select(Role).filter_by(tenant_id=test_tenant.id)).scalar_one()
    ...
    def test_get_user(self, client, test_user_with_role, test_tenant):
        ...
        # Test endpoint
        response = client.get(f"/api/users/{test_user_with_role.id}", headers=headers)
    ...
    def test_update_user(self, client, test_user_with_role, test_tenant):
        ...
        response = client.put(
            f"/api/users/{test_user_with_role.id}", json=update_data, headers=headers
        )
    ...
    def test_update_user_role(self, client, test_user_with_role, test_tenant):
        ...
        updated_user_role = db.session.execute(
            db.select(UserTenantRole).filter_by(
                user_id=test_user_with_role.id, tenant_id=test_tenant.id
            )
        ).scalar_one()
    ...
    def test_permission_denied_manage_users(self, client, test_user_with_role, test_tenant):
        ...
        response = client.put(
            f"/api/users/{test_user_with_role.id}", json={"first_name": "Updated"}, headers=headers
        )
        assert response.status_code == 403
