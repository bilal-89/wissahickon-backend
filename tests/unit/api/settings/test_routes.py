...
def test_get_settings(client, auth_headers, test_tenant, test_settings):
    ...
    response = client.get(f"/api/settings/tenant/{test_tenant.id}", headers=auth_headers)
...
def test_update_settings(client, auth_headers, test_tenant):
    ...
    test_settings = {"theme": "dark", "notifications": True, "language": "en"}
...
def test_delete_setting(client, auth_headers, test_tenant, test_settings):
    ...
    response = client.delete(f"/api/settings/tenant/{test_tenant.id}/theme", headers=auth_headers)
