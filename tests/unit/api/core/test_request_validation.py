...
def test_valid_request(client):
    ...
    response = client.post(
        "/test", json={"email": "test@example.com", "name": "John Doe", "age": 30}
    )
...
def test_invalid_email_format(client):
    ...
    response = client.post("/test", json={"email": "not-an-email", "name": "John Doe"})
    assert response.status_code == 400
...
def test_string_too_long(client):
    ...
    response = client.post(
        "/test", json={"email": "test@example.com", "name": "x" * 101}  # exceeds max_length of 100
    )
    assert response.status_code == 400
...
def test_string_too_short(client):
    ...
    response = client.post(
        "/test", json={"email": "test@example.com", "name": "a"}  # shorter than min_length of 2
    )
    assert response.status_code == 400
