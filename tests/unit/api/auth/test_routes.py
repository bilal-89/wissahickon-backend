...
class TestAuthRoutes:
    ...
    def test_login_rate_limit(self, client, test_user, test_tenant, rate_limiter, fake_redis):
        ...
        response = client.post(
            "/api/auth/login",
            json={"email": "test@example.com", "password": "password123"},  # Correct password
            headers=headers,
        )
    ...
    def test_switch_tenant(self, client, test_user, test_tenant):
        ...
        tenant_role2 = UserTenantRole(
            user_id=test_user.id, tenant_id=tenant2.id, role_id=role2.id, is_primary=False
        )
        ...
        try:
            ...
            response = client.post(
                "/api/auth/switch-tenant", json={"tenant_id": tenant2.id}, headers=headers
            )
