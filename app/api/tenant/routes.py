...
def list_tenants():
    ...
    try:
        ...
        tenant_roles = [tr for tr in user.tenant_roles
        ...
        if primary_tenant_role and Permission.VIEW_TENANT.value not in primary_tenant_role.role.permissions:
            primary_tenant_role = None

        return jsonify({
            'tenants': [tr.tenant.to_dict() for tr in tenant_roles],
            'primary_tenant': primary_tenant_role.tenant.to_dict() if primary_tenant_role else None
        })
    ...
    try:
        ...
        try:
            ...
            user_role = UserTenantRole(
                ...
                is_primary=not UserTenantRole.query.filter_by(user_id=user.id, is_primary=True).first()
            )
...
def list_tenant_users(tenant_id):
    ...
    try:
        ...
        return jsonify({
            'users': [tr.user.to_dict() for tr in tenant_users]
        })
